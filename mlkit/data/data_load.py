...
class DataFrameAdapter(ABC, Generic[DF]):
    ...
    @abstractmethod
    def write_file(self, df: DF, path: Path, file_format: str, **kwargs) -> None:
        """Write data to file."""
        pass
...
class PolarsAdapter(DataFrameAdapter[pl.DataFrame]):
    ...
    def write_file(self, df: pl.DataFrame, path: Path, file_format: str, **kwargs) -> None:
...
class PandasAdapter(DataFrameAdapter[pd.DataFrame]):
    ...
    def write_file(self, df: pd.DataFrame, path: Path, file_format: str, **kwargs) -> None:
...
class DataLoader:
    """Data loader with YAML configuration support."""
    ...
    def read(self, custom_path: Union[str, Path] = None, use_col: Optional[List[str]] = None, **kwargs) -> DF:
        ...
        # Use columns from argument if provided, otherwise from config
        ...
        try:
            df = self.adapter.read_file(path, self.config.file_format, use_col=columns_to_use, **options)
    ...
    def save(self, df: DF, path: Union[str, Path] = None, **kwargs) -> None:
        ...
        try:
            self.adapter.write_file(df, save_path, self.config.file_format, **options)
